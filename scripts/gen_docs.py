...
def main():
    ...
    a("")
    a("Every part below that can be fitted turned now carries a filled")
    a("triangle on the front silkscreen, printed just outside its outline and")
    a("pointing at pin 1.  It is drawn from the real pad, so it is right by")
    ...
    a("| SW1 | 6-way DIP switch | \"ON\" printing on the same side as the silkscreen table |")
    a("| U5 | the converter, hand-fitted | pin 1 is the square pad, at the left, marked on the silkscreen |")
    a("| F1 | PTC fuse | not polarised |")
    a("")
    a("The three BNCs and the converter are through-hole and can be checked")
    ...
    a(f"{len(tht_refs)} parts are through-hole: **{', '.join(tht_refs)}** "
      f"-- the three BNC jacks, the r trimmer, the DC/DC module and the "
      f"USB-C shell tabs.")
    ...
    a("| RV1 | terminal 1 at the bottom, terminal 3 at the top, wiper to the right |")
    ...
    a("6. Try *fast!* (all switches off), *slow!* (4, 5, 6 only) and")
    a("   *glacial!* (all six on).")
