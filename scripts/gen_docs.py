...
def main():
    ...
    a("### Still unverified")
    a("")
    a("These could not be read off JLC's preview and are **not** corrected.")
    a("Check them in the Component Placements view before paying, and if any")
    a("of them is turned, say so and it goes in the table above:")
    a("")
    for code, why in sorted(parts.JLC_UNVERIFIED.items()):
        a(f"* **{code}** -- {why}.")
    a("")
    a("### The unknown-part checkerboards")
    a("")
    a("A checkerboard in the Component Placements view means JLCPCB has no")
    a("drawing of that part to show you.  It does not mean the part is")
    a("unavailable -- that would show up in the BOM tab -- and it does not")
    a("stop the order.  What it costs you is the ability to check that part's")
    a("orientation in the preview, which matters for exactly one of the two")
    a("this board hits:")
    a("")
    a("* **R7 (374k, C2933427)** -- harmless.  It is an 0805 resistor: it has")
    a("  no orientation to get wrong.  Any 374k 1 % 0805 would do, since only")
    a("  the value matters (b = 1M/374k = 2.674), so swap it for one JLC can")
    a("  draw if the checkerboard bothers you.")
    a("* **D1 (MHPA3528CRGBCT, C2962095)** -- not harmless: it is the RGB lamp")
    a("  and it is polarised.  Two ways to cover it:")
    a("")
    a("  1. Put a note on the order: *\"D1 is a common-anode RGB LED; pin 1")
    a("     (the anode) is the corner marked by the filled triangle on the")
    a("     front silkscreen, at the bottom left of the part in")
    a("     lorenz-assembly-top.pdf.\"*  That drawing prints 1:1 and the")
    a("     triangle is in the gerbers, so both sides are looking at the same")
    a("     picture.")
    a("  2. Or deselect D1 from the assembly and solder it by hand.  It is one")
    a("     PLCC-4 on four pads and takes a minute; the rest of the board is")
    a("     unaffected, and a lamp fitted backwards simply never lights.")
    a("")
